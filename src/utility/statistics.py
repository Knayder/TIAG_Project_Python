...
class Statistics:
    ...
    def number_of_abcd_nodes(self, label):
        return len(list(filter( lambda x: str(x.get_label()) == label, self.get_list_of_node())))
    ...
    def averge_vertex_degree_abcd(self, label):
        if self.number_of_abcd_nodes(label) == 0: return 0 
        counter = 0
        for edge in self.graph.get_edge_list():
            ...
            if str(source_label) == label: counter += 1
            if str(destination_label) == label: counter += 1
        return round(counter/self.number_of_abcd_nodes(label),2)
